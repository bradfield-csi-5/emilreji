...
int main() {
  ...
  while (currByte < BUFSIZE) {
    ...
    int currByteInHTTP = currByteInFrame + ipHeaderLength + tcpHeaderLength;

    int httpDataSize = currPacketSize - ipHeaderLength - tcpHeaderLength;
    //     printf("httpDataSize: %d - %d - %d = %d\n", currPacketSize,
    //     ipHeaderLength,
    //            tcpHeaderLength, httpDataSize);
    ...
    if (srcIpAddress == 2757648607 && srcPort == 80) {
      ...
      if (httpDataSize > 0) {
        printf("Payload coming back with data.\n");
      }
    }
...
/*
Option 1:
Create a struct for each httpRecord
Each one has startByte, httpDataSize, and sequence number
Then once you have those structs in an array, sort them by sequence number

Option 2:
page 119 of The C Programming Language
